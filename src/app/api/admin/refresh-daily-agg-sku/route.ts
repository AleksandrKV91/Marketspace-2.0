...
export async function POST(req: NextRequest) {
  ...
  // bySkuMs: sku_ms → sorted by metric_date
  ...
  for (const [skuMs, rows] of Object.entries(bySkuMs)) {
    // Сортировка по дате (уже должна быть, но для надёжности)
    ...
    // Маржа % — берём из первой строки с непустым значением (снапшот)
    const marginPct = rows.find(r => r.margin_pct != null)?.margin_pct ?? 0

    // Начальный остаток (день 0): сумма складов из снапшота первого дня
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Цена на этот день: последняя известная из fact_price_changes
      const price = getPriceOnDate(skuWb, date)

      // Продажи шт = выручка / цена
      ...
      // Остаток сегодня: день 0 = начальный; день N = остаток вчера - продажи вчера
      const stockQty = i === 0
        ? prevStockQty
