...
import { parseAnalytics, type ParseAnalyticsResult } from '@/lib/parsers/parseAnalytics'
...
export async function POST(req: NextRequest) {
  ...
  const filename = req.nextUrl.searchParams.get('filename') ?? 'analytics.xlsx'
  const ct = req.headers.get('content-type') ?? ''

  let parsed: ParseAnalyticsResult

  if (ct.includes('application/json')) {
    try {
      const body = await req.json()
      if (!body.parsed) return NextResponse.json({ error: 'Поле parsed отсутствует в JSON' }, { status: 400 })
      parsed = body.parsed as ParseAnalyticsResult
    } catch (e) {
  ...
  const { data: upload, error: uploadErr } = await supabase
    .from('uploads')
    .insert({ file_type: 'analytics', filename, rows_count: deduped.length, status: 'ok' })
    .select('id')
    .single()
  ...
  for (const batch of chunk(deduped, 1000)) {
    ...
    const { error } = await supabase.from('fact_analytics').upsert(payload, { onConflict: 'sku_ms' })
    if (error) {
