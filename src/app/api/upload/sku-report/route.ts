...
// Параллельный upsert с ограничением concurrency. Снимает «Connection to the database timed out» —
// частые серийные апсерты исчерпывают пул соединений Supabase.
...
export async function POST(req: NextRequest) {
  ...
  const filename = req.nextUrl.searchParams.get('filename') ?? 'sku-report.xlsb'
  const ct = req.headers.get('content-type') ?? ''

  let parsed: ParseSkuReportResult

  // ── Новый flow: client-side parsing → JSON в body ──────────────────────
  // Преимущество: НЕТ скачивания файла из storage (минус большой egress)
  if (ct.includes('application/json')) {
    try {
      const body = await req.json()
      if (!body.parsed) return NextResponse.json({ error: 'Поле parsed отсутствует в JSON' }, { status: 400 })
      parsed = body.parsed as ParseSkuReportResult
    } catch (e) {
      return NextResponse.json({ error: `Ошибка чтения JSON: ${String(e)}` }, { status: 400 })
    }
  } else {
    // ── Legacy flow: storage download → backend parse (на случай если фронт упал) ─
    ...
    // Загружаем skuMap для backend-парсинга
    const skuMap = new Map<string, string>()
    let from = 0
    while (true) {
      const { data } = await supabase.from('dim_sku').select('sku_wb,sku_ms').not('sku_wb', 'is', null).range(from, from + 999)
