...
### A2. fact_price_changes ПУСТА ✅ ROUTE ГОТОВ

**Причина:** `parseStock` заполняет `fact_price_changes` при загрузке Таблицы остатков, но данные не попали.

**Статус:** route `/api/upload/stock` уже сохраняет `price_changes` (строки 74–80). Данные заполнятся при следующей загрузке Таблицы остатков.
...
### D2. Расширенные фильтры в FilterBar (по вкладкам) ✅ ВЫПОЛНЕНО (07.04.2026)

**OrderTab** ✅: Горизонт 30/60/90д | Статус | ABC  
**PriceTab** ✅: Δ CTR | Δ CR заказ | Δ CPM | CPO (>200/≤200)  
**AnalyticsTab** ✅: Маржа | ДРР vs Маржа | Мин. выручка (>100К/>500К/>1М)  
**NicheTab** — в очереди

---

## БЛОК E — ДОРАБОТКА ВКЛАДОК (приоритет #5) ✅ ВЫПОЛНЕНО (06.04.2026)

### E1. OverviewTab («Свод») ✅
- [x] ТОП-15 SKU таблица внизу: # | Название | Арт. WB | Выручка | Маржа% | ABC
- [x] Кнопка Excel для таблицы ТОП-15
- [ ] Главный график: добавить Расходы (3-я линия) — данные в API ещё не проброшены

### E2. AnalyticsTab («Продажи и экономика») ✅
- [x] KPIBar: 7-я карточка «Прогноз 60д» (синяя)
- [ ] Иерархическая таблица Категория → SKU — отложено (требует данных fact_sku_daily)

### E3. PriceTab («Реклама и воронка») ✅
- [x] 3-й график: цена до/после (горизонтальный bar chart, топ-10)
- [x] Таблица: добавлены Δ CPM | Δ CPC колонки
- [x] Сортировка по всем колонкам таблицы

### E4. OrderTab («Логистика и заказы») ✅
- [x] KPIBar: 6-я карточка «Прогноз продаж 60д» (синяя)

### E5. SkuTableTab («Аналитика по SKU») ✅
- [x] Боковая панель (240px, glass): □ OOS критично | □ ДРР>Маржа | □ Маржа<15% | □ С рекламой + [Сбросить]
- [x] Кнопка «Доп. фильтры» с индикатором активности

### E6. NicheTab («Анализ ниш и ABC») ✅
- [x] KPIBar: 5-я карточка «Ниш всего» (синяя)
- [x] График рейтинга ниш: горизонтальные бары, цвет по ABC (зел/жёлт/красный)
- [x] Сортировка по всем колонкам таблицы

---

## БЛОК F — STICKY HEADER (приоритет #6) ✅ ВЫПОЛНЕНО (07.04.2026)

- [x] `.top-nav` → `position: sticky; top: 0; z-index: 50` (в globals.css)
- [x] Вкладки-таблицы: `sticky-thead` класс → `thead th { position: sticky; top: var(--header-h) }` — добавлен во все 5 таблиц
- [x] `--header-h: 88px` CSS переменная

---

## БЛОК G — ГЛОБАЛЬНЫЙ КАЛЕНДАРНЫЙ ФИЛЬТР ✅ ПОЛНОСТЬЮ ВЫПОЛНЕНО (07.04.2026)
- [x] Компонент DateRangePicker — в header row 2, под логотипом слева
- [x] Dropdown открывается вниз (`left-0`, `top: calc(100%+8px)`)
- [x] from/to передаётся во все API routes: analytics, prices, sku-table, orders
- [x] При смене диапазона — все вкладки автоматически перезагружают данные
