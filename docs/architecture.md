...
>
> Дата создания: 2026-04-27 · Last sync: 2026-05-19 (миграции 020, 022 + helper `lib/novelty.ts`).

---

## ⚠️ Миграция 022 — серверная агрегация для «Свод», «Аналитика по SKU», «Реклама и воронка»

С версии `022_overview_sku_rpcs.sql` роуты `/api/dashboard/{overview,sku-table,prices}` перешли с `fetchAll<fact_sku_daily>` (тащил сотни тысяч строк → «TypeError: Failed to fetch» на 30+ днях) на агрегирующие RPC:

- `sku_period_full_agg(p_from, p_to, p_prev_from, p_prev_to)` — per-SKU агрегаты выручки, ad_spend, ЧМД, маржи + средние CTR/CR/CPM/CPC за текущий и предыдущий период.
- `sku_daily_full_agg(p_from, p_to)` — per-date агрегаты для трендового графика «Свода».
- `prices_funnel_period_agg(p_from, p_to, p_prev_from, p_prev_to)` — KPI воронки за период.
- `prices_daily_funnel_agg(p_from, p_to)` — per-date агрегаты воронки + средневзвешенная цена.

Все четыре функции вызываются через `rpcFetchAll` (постраничный workaround серверного `max-rows = 1000`). Если миграция не применена — роуты возвращают 503 с подсказкой запустить SQL.

`maxDuration` всех затронутых роутов поднят до 300 секунд.

---

## ⚠️ Helper `lib/novelty.ts` — фильтр «Новинки»

Колонка I «Отчёта по SKU» содержит `Новинка` (в единственном числе), а старый код сравнивал с `'Новинки'` — фильтр всегда возвращал 0. Теперь во всех роутах и компонентах используются `isNovelty(status)` / `matchesNoveltyFilter(status, filter)`, толерантные к `Новинка`, `Новинки`, `НОВИНКА`, `new`.
...
| ~~`overview`/`prices`/`sku-table` таймаут на 30+ днях~~ | ~~`fetchAll<fact_sku_daily>` тянул 300К+ строк → клиент ловил «Failed to fetch».~~ | **исправлено 2026-05-19**: миграция 022 + переход на агрегацию через RPC. |
| ~~Фильтр «Новинки» → нули~~ | ~~Сравнение `=== 'Новинки'` не находило `Новинка` из Excel.~~ | **исправлено 2026-05-19**: helper `lib/novelty.ts`. |
